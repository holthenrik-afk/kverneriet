...
def index_page():
    G.add('blog.title', 'Blogg', 'Blog'); G.add('blog.eyebrow', 'Nytt fra Kverneriet', 'News from Kverneriet'); G.add('blog.read', 'Les innlegget', 'Read the post')
    G.add('blog.empty', 'Her kommer nyheter, nye retter og historier fra kjøkkenet. Følg med.', 'News, new dishes and stories from the kitchen will appear here. Stay tuned.')
    body = f'<div class="grid-3" style="margin-top:var(--space-7)">{"".join(card(p) for p in POSTS)}</div>' if POSTS else '<p class="lede" style="margin-top:var(--space-5)" data-i18n="blog.empty">Her kommer nyheter, nye retter og historier fra kjøkkenet. Følg med.</p>'
    return head('Blogg – nytt fra Kverneriet', 'Nyheter, nye retter og historier fra kjøkkenet på Kverneriet i Oslo og Tønsberg.') + f'''<body>

{chrome.header()}
<main id="main">
  <div class="wrap" style="padding-top:var(--space-8);padding-bottom:var(--section-y)">
    <nav class="crumbs" aria-label="Brødsmuler"><ol><li><a href="/">Kverneriet</a></li><li><span aria-current="page" data-i18n="blog.title">Blogg</span></li></ol></nav>
    <header class="sec-head">
      <div class="sec-head__rule"></div>
      <span class="kv-eyebrow" data-i18n="blog.eyebrow">Nytt fra Kverneriet</span>
      <h1 class="hero-title" style="font-size:var(--type-display-1)" data-i18n="blog.title">Blogg</h1>
    </header>
    {body}
  </div>
</main>
